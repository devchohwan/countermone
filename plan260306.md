...
pass_reason       text
from_pass         boolean   DEFAULT false
                            -- 패스 사용으로 자동 생성된 추가 Schedule 여부
                            -- 패스 취소/전환 시 이 Schedule을 찾아 삭제하기 위함
created_at        datetime
updated_at        datetime
```
...
- [x] 레이아웃 shell 구성 (application.html.erb)
  - 상단 Navbar (DaisyUI navbar)
  - 좌측 사이드바 (DaisyUI drawer)
  - 메인 콘텐츠 영역
  - 다크모드 토글 (DaisyUI Theme Controller, data-theme 토글)
- [x] Flash 메시지 → DaisyUI Toast 컴포넌트 연결
- [x] Turbo Stream 기본 설정

**완료 기준**
- [x] 서버 기동 시 레이아웃 shell이 렌더링됨
- [x] 다크모드 토글 작동
- [x] Flash → Toast 표시됨
...
**완료 기준**
- [x] 결제 등록 시 해당 enrollment 기준으로 Schedule 자동 생성됨
- [x] 수강생 상세에서 클래스별 수업 이력 확인 가능
- [x] 잔여 횟수가 Schedule 집계로 자동 계산됨
- [x] 2개 이상 클래스 수강 시 각각 독립적으로 관리됨
...
- [x] 출결 처리 — 대시보드 ① 섹션에서 직접 처리 (별도 페이지 없음)
...
**완료 기준**
- [x] 키패드 코드 입력 시 Schedule.status → attended로 변경됨
- [x] N개 클래스 수강생은 클래스 선택 화면이 표시됨
- [x] 오류 발생 시 상담원 화면에 Toast 알림 표시
- [x] 대시보드에서 등원/하원/결강 처리 가능
...
- [x] 시간표 메모란 (자리 대기, 일정 변경 대기)

**완료 기준**
- [x] 선생님별 주간 시간표 렌더링됨
- [x] 슬롯 3명 풀이면 초록 표시 및 추가 배치 불가

---

### 4단계 — 보강/패스/차감 관리

**목표**: 스케줄 변동 처리. 보강/패스/차감은 보강 가능 기간 내에 언제든 유기적으로 전환 가능.
...
- 패스 적용 시:
  ...
  - 결제분 마지막 Schedule.lesson_date + 7일에 Schedule 추가 생성 (status: scheduled, from_pass: true)
  - consecutive_weeks 리셋 → 상담원에게 경고 표시 ("패스 사용 시 개근 횟수가 리셋됩니다")
- 당일 패스 시도 → 불가. 보강 유도 → 보강도 불가하면 차감으로 3단계 안내
- 믹싱 패스 불가 → 동일하게 보강 유도 → 불가하면 차감
- 2주 이상 선패스: 잔여 횟수 충분하면 허용, 부족하면 요청 주차 이전에 다시 신청 안내
- 긴급패스 → Schedule.status → emergency_pass (상담원 수동, 추가 Schedule 생성 동일하게 적용)
...
- 보강 가능 선생님: teacher_subjects 기준 동일 과목 담당 선생님으로만 제한
- 담당 선생님 슬롯 우선 표시, 불가 시 같은 과목 타 선생님 슬롯 표시
- 보강 확정 시 Schedule.makeup_date, makeup_time, makeup_teacher_id, status → makeup_scheduled
- 당일 보강 취소: 경고 표시 후 상담원 확인으로 진행 (막지 않음)
- 믹싱 1차전직 보강 3단계:
  1. 같은 주차인 미라쿠도 반 슬롯 확인
  2. 없으면 미라쿠도 빈 시간으로 조율
  3. 그것도 불가하면 오또 빈 시간 배정 (드라이브 기입: 홍길동(01.09지명믹싱오또보강ㅇ회차))
- 믹싱 2차전직 보강: makeup_approved 플로우 (미라쿠도 확인 후 상담원이 승인)

---

#### 차감 처리 규칙
- 결석 확정 50분에 상담원이 수동 차감 (Schedule.status → deducted)
- 차감 취소 액션 (`patch :undo_deduct`) 별도 제공
  - 차감 취소 후 보강/패스 전환 가능 (보강 가능 기간 체크 필요)
  - 기간 만료 시 차감 취소만 가능, 보강/패스 전환 불가 → 상담원 알림

---

#### 보강/패스/차감 전환 로직 (핵심)

보강 가능 기간 내에는 언제든 전환 가능. 상태 전환 시 공통 처리:

**→ 패스로 전환 시**
- 잔여 패스 횟수 체크 (0이면 전환 불가, 경고 표시)
- from_pass: true인 추가 Schedule 생성
- 기존 보강/차감 상태 해제

**→ 보강으로 전환 시**
- 보강 가능 기간 체크
- 슬롯 체크 (꽉 찬 경우 → 상담원이 브레이크타임 개방 또는 예외 처리)
- from_pass: true인 추가 Schedule 있으면 삭제
- 기존 패스/차감 상태 해제
...
**보강 완료(makeup_done) 후 전환 시도**
- 경고 표시 후 상담원 확인으로만 진행 (막지 않음)
...
| 전환 | 처리 |
|---|---|
| 패스 → 보강 | 추가 Schedule 삭제 + makeup_scheduled |
| 패스 → 차감 | 추가 Schedule 삭제 + deducted |
| 보강 → 패스 | 패스 잔여 확인 + 추가 Schedule 생성 |
| 보강 → 차감 | deducted |
| 차감 → 보강 | undo_deduct + 기간/슬롯 체크 + makeup_scheduled |
| 차감 → 패스 | undo_deduct + 패스 잔여 확인 + 추가 Schedule 생성 |
| 패스 → 보강 → 차감 | 추가 Schedule 삭제(패스→보강 시 이미 삭제) + deducted |
| 패스 → 보강 → 날짜변경 | makeup_date 업데이트 + 슬롯 체크 |
| 패스 → 차감 → 보강 | undo_deduct + 기간/슬롯 체크 |
| 패스 → 차감 → 패스 | undo_deduct + 잔여 확인 + 추가 Schedule 재생성 |
| 보강 → 패스 → 차감 | 추가 Schedule 삭제 + deducted |
| 보강 → 패스 → 보강 | 추가 Schedule 삭제 + makeup_scheduled |
| 보강 → 차감 → 패스 | undo_deduct + 잔여 확인 + 추가 Schedule 생성 |
| 보강 → 차감 → 보강 | undo_deduct + 기간/슬롯 체크 |
| 차감 → 보강 → 패스 | 잔여 확인 + 추가 Schedule 생성 |
| 차감 → 보강 → 차감 | deducted |
| 차감 → 패스 → 보강 | 추가 Schedule 삭제 + 슬롯 체크 |
| 차감 → 패스 → 차감 | 추가 Schedule 삭제 + deducted |

---

**작업 목록**
- [x] schedules 테이블에 from_pass boolean 컬럼 추가
- [x] 패스 처리 액션 (pass, emergency_pass)
  - 추가 Schedule 생성 로직 포함
  - 개근 리셋 경고 UI
- [x] 보강 처리 액션 (makeup, complete_makeup)
  - 믹싱 1차전직 3단계 플로우
  - 믹싱 2차전직 makeup_approved 플로우
- [x] 차감 처리 액션 (deduct, undo_deduct)
  - undo_deduct: 차감 취소 후 보강/패스 전환 가능 상태로 복귀
- [x] 전환 공통 처리 모듈 (TransitionService)
  - 어떤 상태에서 어떤 상태로 가든 위 전환 테이블 기준으로 처리
  - 잔여 패스 횟수 체크
  - 보강 가능 기간 체크
  - 슬롯 체크
  - from_pass Schedule 삭제/생성
- [x] 당일 패스 시도 → 보강 유도 → 차감 3단계 UI 흐름
- [x] 선생님별 패스 사용 시트 자동 기록

**완료 기준**
- [x] 보강/패스/차감이 보강 가능 기간 내 자유롭게 전환됨
- [x] 패스 사용 시 추가 Schedule 생성됨
- [x] 패스 취소/전환 시 추가 Schedule 삭제됨
- [x] 차감 취소(undo_deduct) 액션 작동
- [x] 믹싱 1차전직 보강 3단계 플로우 작동
- [x] 믹싱 2차전직 보강 승인 플로우 작동
...
**완료 기준**
- [x] 오늘 결제/연락 대상이 자동 리스트업됨
- [x] 12주 개근 달성 시 상담원 알림
- [x] 지인 할인이 추천인 다음 결제에 자동 적용됨
...
- [x] ① 지금 이 순간 섹션 (Turbo Stream, 가장 크게)
...
**완료 기준**
- [x] 출근 후 대시보드 하나로 오늘 해야 할 일이 전부 보임
- [x] 출석 처리 시 ① 섹션 실시간 갱신 (Turbo Stream)
- [x] 다크모드 토글 작동 (DaisyUI Theme Controller)
- [x] 대시보드에서 등원/하원/결강/결제/연락완료 처리 가능
...
**완료 기준**
- [x] 마감 톡 화면에서 숫자 자동 채워지고 특이사항만 입력하면 됨
- [x] 월별 매출 집계 정확함
