...
* **기능 완성 후 반드시 작동 검사는 ralph loop로** — `/ralph`로 검증하고, 문제 없음이 2회 연속 확인되면 완료. curl/rails runner 단독 검증 후 바로 배포하지 말 것. 테스트 데이터가 없으면 rails runner로 직접 생성한 뒤 검증하고, 검증 후 테스트 데이터는 삭제한다. **ralph 검증은 실제 등록 플로우까지 포함해야 한다** — 파일 존재/문자열 포함 같은 정적 분석만으로는 부족하다. rails runner로 테스트 데이터를 생성하고, 컨트롤러 로직을 직접 호출하거나 시뮬레이션하여 DB 결과까지 확인한 후 테스트 데이터를 삭제한다
...
## 배포 규칙
* **`kamal deploy`는 사용자가 명시적으로 요청할 때만 실행한다** — 코드 변경/커밋 후 자동으로 배포하지 말 것
